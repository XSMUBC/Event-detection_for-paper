...
dset1= [0.89, 0.95, 0.97, 0.99, 1.00, 1.00, 1.00, 1.00, 1.00,1.00]
dset2= [0.80, 0.90, 0.95, 0.97, 0.99, 1.00, 1.00, 1.00, 1.00,1.00]
dset3= [0.68, 0.85, 0.89, 0.92, 0.94, 0.96, 1.00, 1.00, 1.00,1.00]
dset4= [0.51, 0.70, 0.81, 0.83, 0.87, 0.90, 0.98, 1.00, 1.00,1.00]
...
spl = make_interp_spline(np.arange(0, 1.0, 0.1),dset1, k=1) #BSpline object
power_smooth = spl(xnew)
plt.plot(xnew,power_smooth, ':r',linewidth=10.0, label='EPCUSUM')


xnew = np.linspace(0,1,800) #300 represents number of points to make between T.min and T.max
spl = make_interp_spline(np.arange(0, 1.0, 0.1),dset2, k=1) #BSpline object
power_smooth = spl(xnew)
plt.plot(xnew,power_smooth, '--y',linewidth=10.0, label='BF+MBCUSUM')


xnew = np.linspace(0,1,800) #300 represents number of points to make between T.min and T.max
spl = make_interp_spline(np.arange(0, 1.0, 0.1),dset3, k=1) #BSpline object
power_smooth = spl(xnew)
...
xnew = np.linspace(0,1,800) #300 represents number of points to make between T.min and T.max
spl = make_interp_spline(np.arange(0, 1.0, 0.1),dset4, k=1) #BSpline object
power_smooth = spl(xnew)
